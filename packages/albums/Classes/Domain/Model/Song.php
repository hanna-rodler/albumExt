...
class Song extends \TYPO3\CMS\Extbase\DomainObject\AbstractEntity
{
  ...
  public function initializeObject(){
    $this->interprets=
      $this->interprets ?: new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
  }
   ...
   * Removes a Interpret
   *
   * @param \HannaRodler\Albums\Domain\Model\Interpret $interpretToRemove The
   *                                                                      Interpret
   *                                                                      to
   *                                                                      be
   *                                                                      removed
  ...
  public function getAllInterprets(){
    $interprets=[];
    foreach($this->interprets as $interpret){
      $interprets[$interpret->getUid()]=$interpret;
    }
    return $interprets;
  }
  
  public function getDurationMinsSecs(){
    $durationMinAndSecs = "";
    $durationMinAndSecs.=intdiv($this->duration, 60);
    $durationMinAndSecs.=":".$this->duration%60;
    $durationMinAndSecs.=" min";
    return $durationMinAndSecs;
  }
}
